...
`Alert`、コンパニオンのバージョン状態のためのテスト済み型付きメッセージです。

**統合の境界：** 認識された音声要求は、ペアリング済みかつ認証済みのトランスポートが
接続されるまでウォッチ上に留まります。音声でロボットを直接動作させることは決して
なく、移動に関係する応答は確認を要求し、物理 E-STOP は独立して維持されます。

**まだ先にあるもの：** 認証済み WebSocket/Data Layer ペアリング、
HYDRA-UMC-VOICE-UI ゲートウェイへの配信、ライブのシステム状態カード、
実機 Wear OS デバイスでのエンドツーエンド検証です。
