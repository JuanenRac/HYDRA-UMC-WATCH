...
- **New `WatchRelayRouting.decide()`** (`transport/WatchRelayRouting.kt`) -
  ...
  `RelayRetryPolicy`/`LastKnownStateCache`, which this project's own
  README explains were deliberately extracted into pure, JVM-testable
  classes for exactly this reason. `onMessageReceived()` is now a thin
  Android adapter calling this pure decision function. 6 new JVM unit
  tests (no emulator/watch needed): a real forwarded `AssistantReply`/
  `SystemStatus`, an unrelated path dropped, malformed bytes dropped, a
  message failing real protocol validation dropped, and a real,
  validly-parseable message of the wrong type (`EStopCommand`) dropped
  even on a matching relay path. `./gradlew test` passes.
...
- **Companion-verified node selection.** `WatchRelayTransport.send()`
  ...
  proof it was actually running the companion app. Two connected nodes,
  only one with HYDRA-UMC-ANDROID-CONTROL installed, could send to the
  wrong one. Now queries a real `CapabilityClient` for the
  `hydra_umc_phone_companion` capability (declared in HYDRA-UMC-ANDROID-
  CONTROL's own new `res/values/wear.xml`) and only ever picks from
  nodes that capability actually verified - `pickCompanionNode()`
  additionally prefers a nearby (directly Bluetooth-reachable) node over
  a cloud-relayed one when more than one qualifies.
- **Retry cancellation on lifecycle teardown.** A deferred retry
  (`sendWithRetry`'s own `handler.postDelayed`) had no way to be
  cancelled - only a max-attempt counter bounded it, so a pending retry
  from before the screen closed still fired later against an already-
  destroyed `MainActivity`. New `WatchRelayTransport.cancelPendingRetries()`,
  called from `MainActivity.onDestroy()`.
- 4 new tests (`PickCompanionNodeTest`) covering no-nodes, a single node,
  nearby-preferred-over-relayed, and the no-nodes-nearby fallback.

Verified with `./gradlew testDebugUnitTest assembleDebug` (this repo's
own real build command) - all unit tests pass and the debug APK builds.
The paired HYDRA-UMC-ANDROID-CONTROL side was also verified with its own
`./gradlew assembleDebug` after adding the new capability declaration.
